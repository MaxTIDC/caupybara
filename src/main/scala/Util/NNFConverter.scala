...
object NNFConverter {
   ...
   */
  def toNNF(phi: LTL, beer2011: Boolean = false): LTL = {
    this.beer2011 = beer2011
    toNNFHelper(toImplFree(phi))
  }
  ...
  /**
   * Helper method for toNNF
   */
  private def toNNFHelper(phi: LTL): LTL = phi match
    // Propositional
    ...
    case F(phi) => if beer2011 then U(True, toNNFHelper(phi)) else F(toNNFHelper(phi))
